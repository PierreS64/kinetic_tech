...
import { CreateTradeInDto, UpdateTradeInStatusDto } from './dtos/trade-in.dto';
...
export class TradeInService {
  ...
  async getTradeInById(id: string) {
    ...
    if (!tradeIn)
      throw new NotFoundException('Yêu cầu thu cũ đổi mới không tồn tại');
    return tradeIn;
  }
  ...
  async updateStatus(id: string, dto: UpdateTradeInStatusDto) {
    return this.prisma.tradeIn.update({
      ...
      data: {
        status: dto.status,
        estimatedValue:
          dto.estimatedValue !== undefined ? dto.estimatedValue : undefined,
      },
    });
  }
}
