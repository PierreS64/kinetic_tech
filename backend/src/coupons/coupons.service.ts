...
export class CouponsService {
  ...
  async applyCoupon(code: string) {
    const coupon = await this.prisma.coupon.findFirst({
      where: { code: code.toUpperCase(), isActive: true, deletedAt: null, validFrom: { lte: new Date() }, validUntil: { gte: new Date() } }
    });
    if (!coupon) throw new NotFoundException('Mã giảm giá không hợp lệ hoặc đã hết hạn');
    return coupon;
  }
